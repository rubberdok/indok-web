...
def create_booking_validation(
    booking_data: BookingModel, booking_semester: BookingSemester
):
...
def booking_semester_validation(
    ...
    dates_in_fall_semester = check_dates_in_range(
        [check_in, check_out],
        booking_semester.fall_start_date,
        booking_semester.fall_end_date,
    )
    dates_in_spring_semester = check_dates_in_range(
        [check_in, check_out],
        booking_semester.spring_start_date,
        booking_semester.spring_end_date,
    )
    not_in_any_booking_semester = (
        not dates_in_spring_semester and not dates_in_fall_semester
    )

    if (
        not booking_semester.fall_semester_active
        and not booking_semester.spring_end_date
    ):
