...
def price(
    ...
    if is_internal_price(internal_participants, external_participants):
        weekday_price_pr_night = cars.aggregate(models.Sum("internal_price"))["internal_price__sum"]
        weekend_price_pr_night = cars.aggregate(models.Sum("internal_price_weekend"))["internal_price_weekend__sum"]
    else:
        weekday_price_pr_night = cars.aggregate(models.Sum("external_price"))["external_price__sum"]
        weekend_price_pr_night = cars.aggregate(models.Sum("external_price_weekend"))["external_price_weekend__sum"]
