        ...
        if previous.is_attending and not instance.is_attending:
            ...
            if attending and event.is_full:
                ...
                if len(users_on_wait_list) > 0:
                    EventEmail.send_waitlist_notification_email(
                        users_on_wait_list[0], event
                    )


@receiver(pre_save, sender=Event)
def send_wait_list_notification_when_events_expand(
    sender: Type[Event], instance: Event, **kwargs
):
    if not instance._state.adding:
