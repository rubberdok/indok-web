...
def permission_required(perms: Union[list[str], str], lookup_variables: tuple[Union[Model, ModelBase, str], ...]=None, **kwargs):
      ...
      if lookup_variables:
        ...
        if isinstance(model, str):
          ...
          if len(split_model_str) != 2:
            raise SyntaxError(f"The model string should be of the format app_label.ModelClass, got {model}.")
        ...
        if len(lookups) % 2 != 0:
          raise ValueError(f"The lookups should come in pairs of strings in the format attribute name, parameter name.")
...
def has_permissions(user, perms: list[str], obj=None, accept_global_perms: bool=False) -> bool:
