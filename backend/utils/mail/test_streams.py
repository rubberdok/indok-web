...
class PostmarkEmailTests(SimpleTestCase):
    def test_warns_when_template_variables_missing_for_multiple_recipients(self):
        ...
        self.assertTrue(
            any("template_variables is None" in str(w.message) for w in caught)
        )
    ...
    def test_sets_default_and_global_template_variables(self):
        ...
        self.assertEqual(
            email.merge_global_data["parent_company"],
            "Foreningen for Studenter ved Indøk",
        )
        self.assertEqual(
            email.merge_global_data["website_url"], settings.FRONTEND_BASE_URL
        )
        self.assertEqual(
            email.merge_global_data["contact_mail"], settings.CONTACT_EMAIL
        )
