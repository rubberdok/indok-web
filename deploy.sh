...
deploy_cluster() {
  ...
  task_def=$(printf "$task_template" $AWS_ACCOUNT_ID $CODEBUILD_SOURCE_VERSION $AWS_RDS_HOST $AWS_RDS_PASSWORD $PRODUCTION_SECRET_KEY $DATAPORTEN_SECRET $GOOGLE_DRIVE_API_KEY $DJANGO_SETTINGS_MODULE $AWS_SECRET_ACCESS_KEY $CODEBUILD_SOURCE_VERSION)
  echo "$task_def"
  register_definition
  update_service
  ...
  task_template=$(cat "ecs/$template")
  task_def=$(printf "$task_template" $AWS_ACCOUNT_ID $CODEBUILD_SOURCE_VERSION)
  echo "$task_def"
  register_definition
  update_service
