...
import { Grid, makeStyles, Theme, Typography, Box, Divider, Tooltip } from "@material-ui/core";
...
const CheckInOut: NextPage<Props> = ({ chosenCabins, datePick, contactInfo }) => {
  ...
  const infos: { info: Cabin[] | DatePick | ContactInfo; renderComponent: ReactNode }[] = [
    {
      ...
      renderComponent: (
        <InfoText>
          Du du booker nå <Box className={classes.bold}>{chosenCabins.map((cabin) => cabin.name)}</Box>
        </InfoText>
      ),
    },
    {
