...
const DeclineBookingDialog: React.VFC<DialogProps> = ({
  ...
  return (
    <Dialog open={bookingToBeDeclined != undefined} onClose={handleDeclineBookingOnClose}>
      <DialogTitle>
        Underkjenning av booking fra {bookingToBeDeclined?.firstName}{" "}
        {bookingToBeDeclined?.lastName} fra {convertDateFormat(bookingToBeDeclined?.checkIn)} til{" "}
        {convertDateFormat(bookingToBeDeclined?.checkOut)} av{" "}
        {toStringChosenCabins(bookingToBeDeclined ? bookingToBeDeclined.cabins : [])}
      </DialogTitle>
      <DialogContent>
        <DialogContentText>
          Er du sikker på at du vil underkjenne denne bookingen?
        </DialogContentText>
        <DialogContentText>
          Det kan være nyttig for {bookingToBeDeclined?.firstName} å få vite hvorfor dere avslår
          søknaden om booking. Hvis dere vil oppgi grunnen til avslag, kan dere gjøre det nedenfor.
        </DialogContentText>
        <TextField
          ...
          onClick={() => {
            if (bookingToBeDeclined) {
              ...
              declineBooking({
                ...
                setSnackbarMessage(
                  `Bookingen er underkjent. Mail er sendt til ${bookingToBeDeclined.receiverEmail}.`
                );
