...
const EventDetailPage: React.FC<Props> = ({ eventId }) => {
  ...
  const { data: eventData, error: eventError, loading: eventLoading, refetch: refetchEventData } = useEventQuery({
  ...
  const handleClick = () => {
    ...
    eventSignUp({ variables: { eventId: eventId.toString(), data: extraInformation } })
      .then(() => {
        refetchEventData({ id: eventId.toString() }).then((res) => {
    ...
    <Grid container spacing={1}>
      ...
      <Grid item xs={4}>
        <Paper variant="outlined" className={classes.paper}>
          ...
          {eventData.event.allowedGradeYears && eventData.event.allowedGradeYears.length < 5 && (
            <Box my={2}>
              <Typography variant="overline" display="block">
                Åpent for
              </Typography>
        ...
        <Paper variant="outlined" className={classes.paper}>
          ...
          {!eventData.event.isAttendable ? null : !userData.user ? (
            <Typography variant="h6" className={classes.signUpText}>
              ...
              eventData.event.allowedGradeYears && eventData.event.allowedGradeYears.includes(userData.user.gradeYear)
            ) ? (
            <Typography variant="h6" className={classes.signUpText}>
              Ikke aktuell
            </Typography>
            ...
            <>
              <CountdownButton
                countDownDate={eventData.event.signupOpenDate}
                ...
                isFull={eventData.event.isFull ?? false}
