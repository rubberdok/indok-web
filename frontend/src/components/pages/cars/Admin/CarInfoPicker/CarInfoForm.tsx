...
const validationSchema: yup.ObjectSchema<CarInfoForm> = yup.object({
  oksen: yup.object({
...
type Props = {
  defaultValues: CarInfoForm;
  onSubmit: (data: CarInfoForm) => void;
  values?: CarInfoForm;
};

export const CarInfoForm: React.FC<Props> = ({ defaultValues, values, onSubmit }) => {
  const {
    register,
    formState: { errors },
    handleSubmit,
